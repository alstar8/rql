#!/usr/bin/env bash
# V17 residual_rlt_cf + flow_rlt_cf (kettle-matched offline, safe collect).
#   residual_rlt_cf → GPUs 0-3 (4 shards)
#   flow_rlt_cf     → GPUs 4-7 (4 shards)
# Ports 8730–8737. Requires runs/rlt_pretrain_house0_kettle_z256_d512_l4/.
...
RUN_DIR="${RUN_DIR:-${ROOT}/runs/rlt_cf_v17_kettle}"
LOCAL_LOG_DIR="${LOCAL_LOG_DIR:-${B1K_TMP}/rlt_cf_v17_kettle_logs}"
...
RLT_EGL_LOCK_DIR="${RLT_EGL_LOCK_DIR:-${B1K_TMP}/rlt_egl_locks_v17_kettle}"
TMP_ROLLOUT_DIR="${TMP_ROLLOUT_DIR:-${B1K_TMP}/molmoact2_rlt_rollouts_v17_kettle}"
...
BASE_HTTP_PORT="${BASE_HTTP_PORT:-8730}"
SHARDS_PER_ARM="${SHARDS_PER_ARM:-4}"

case "${V17_MODE}" in
  full)
    V17_MAX_VALID_EPISODES="${V17_MAX_VALID_EPISODES:-400}"
    V17_TARGET_ENV_STEPS="${V17_TARGET_ENV_STEPS:-250000}"
    V17_SNAPSHOT_EPISODES="${V17_SNAPSHOT_EPISODES:-0,100,200,400}"
    ;;
  long)
    V17_MAX_VALID_EPISODES="${V17_MAX_VALID_EPISODES:-1000}"
    V17_TARGET_ENV_STEPS="${V17_TARGET_ENV_STEPS:-600000}"
    V17_SNAPSHOT_EPISODES="${V17_SNAPSHOT_EPISODES:-0,100,200,400,700,1000}"
    ;;
  smoke)
    V17_MAX_VALID_EPISODES="${V17_MAX_VALID_EPISODES:-2}"
    V17_TARGET_ENV_STEPS="${V17_TARGET_ENV_STEPS:-1000}"
    V17_SNAPSHOT_EPISODES="${V17_SNAPSHOT_EPISODES:-0,${V17_MAX_VALID_EPISODES}}"
    ;;
  *)
    echo "[v17] V17_MODE must be full, long, or smoke" >&2
    exit 1
    ;;
esac

V17_MAX_UPDATE_SEC_PER_EPISODE="${V17_MAX_UPDATE_SEC_PER_EPISODE:-60}"
V17_UPDATES_PER_EPISODE="${V17_UPDATES_PER_EPISODE:-128}"
...
if [[ "$(basename "${RUN_DIR}")" != "rlt_cf_v17_kettle" ]]; then
  echo "[v17] RUN_DIR basename must be rlt_cf_v17_kettle" >&2
  exit 1
fi
if [[ ! -f "${RESIDUAL_CKPT}" || ! -f "${FLOW_CKPT}" ]]; then
  echo "[v17] missing kettle pretrain ckpts under ${PRETRAIN_DIR}" >&2
  echo "  run: bash pretrain_rlt_house0_kettle_d512_l4.sh" >&2
  exit 1
fi
if [[ ! -x "${PYTHON}" || ! -x "${SERVE_PYTHON}" ]]; then
  echo "[v17] python missing" >&2
  exit 1
fi
...
export V17_MODE V17_MAX_VALID_EPISODES V17_TARGET_ENV_STEPS V17_SNAPSHOT_EPISODES
export V17_MAX_UPDATE_SEC_PER_EPISODE V17_UPDATES_PER_EPISODE
export RLT_CF_V17_RUN_DIR="${RUN_DIR}"
...
if (( ${#GPU_ARRAY[@]} != 8 )); then
  echo "[v17] need 8 GPUs, got ${#GPU_ARRAY[@]}" >&2
  exit 1
fi
...
pid_belongs_to_run() {
  ...
  while IFS= read -r entry; do
    [[ "${entry}" == "RLT_CF_V17_RUN_DIR=${RUN_DIR}" ]] && return 0
  done < <(tr '\0' '\n' < "${environ}")
  return 1
}
...
wait_for_server() {
  ...
  echo "[v17] server ${variant} port ${port} not ready" >&2
  return 1
}
...
start_server() {
  ...
  echo "[v17 $(date -Is)] server ${variant} s${shard} gpu=${gpu} port=${port}"
  (
    cd "${MOLMOACT2}"
    exec setsid env \
...
start_trainer() {
  ...
  echo "[v17 $(date -Is)] train ${variant} s${shard} gpu=${gpu} port=${port} fresh=${fresh_start}"
  (
    exec setsid env \
      RLT_CF_V17_RUN_DIR="${RUN_DIR}" \
      CUDA_VISIBLE_DEVICES="${gpu}" \
...
launch_arm() {
  ...
  if (( ${#gpus[@]} != SHARDS_PER_ARM )); then
    echo "[v17] expected ${SHARDS_PER_ARM} GPUs for ${variant}" >&2
    return 1
  fi
  if [[ "${FRESH}" == "1" ]]; then
...
echo "[v17 $(date -Is)] launching residual_rlt_cf on GPUs ${RESIDUAL_GPUS[*]}"
launch_arm residual_rlt_cf "${RESIDUAL_CKPT}" "${RESIDUAL_GPUS[@]}"
echo "[v17 $(date -Is)] launching flow_rlt_cf on GPUs ${FLOW_GPUS[*]}"
launch_arm flow_rlt_cf "${FLOW_CKPT}" "${FLOW_GPUS[@]}"

touch "${RUN_DIR}/.initial_launch_complete"
echo "[v17 $(date -Is)] initial launch complete; polling every ${POLL_SEC}s"
