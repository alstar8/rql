"""Controlled V16 harness: paper Alg.1 always-on collect + strong soft-β BC.

Delegates launch/eval CLI machinery to v14_harness after installing V16
...
RUN_NAME = "rlt_cf_v16_controlled"
...
MAX_VALID_EPISODES = int(os.environ.get("V16_MAX_VALID_EPISODES", "400"))
TARGET_ENV_STEPS = int(os.environ.get("V16_TARGET_ENV_STEPS", "250000"))
SNAPSHOT_EPISODES = tuple(
    int(token.strip())
    for token in os.environ.get(
        "V16_SNAPSHOT_EPISODES",
        "0,100,200,400",
    ).split(",")
    if token.strip()
)
AE_IMAGE_REPLAY_CAPACITY = v14.AE_IMAGE_REPLAY_CAPACITY
AE_BATCH_SIZE = int(os.environ.get("V16_AE_BATCH_SIZE", str(v14.AE_BATCH_SIZE)))
AE_MICROBATCH_SIZE = int(
    os.environ.get("V16_AE_MICROBATCH_SIZE", str(v14.AE_MICROBATCH_SIZE))
)
AE_MIN_SUCCESS_EPISODES = int(
    os.environ.get("V16_AE_MIN_SUCCESS_EPISODES", str(v14.AE_MIN_SUCCESS_EPISODES))
)
...
AE_UPDATES_PER_EPISODE = int(os.environ.get("V16_AE_UPDATES_PER_EPISODE", "32"))
RUN_MODE = os.environ.get("V16_MODE", "full")

# Paper-aligned defaults: no gate-off mixture; collect actor after warmup.
ACTOR_MIXTURE_PROB = float(os.environ.get("V16_ACTOR_MIXTURE_PROB", "0.0"))
ACTOR_BC_EPISODES = int(os.environ.get("V16_ACTOR_BC_EPISODES", "50"))
RESIDUAL_CLIP = float(os.environ.get("V16_RESIDUAL_CLIP", "0.02"))
ADVANTAGE_CLIP = float(os.environ.get("V16_ADVANTAGE_CLIP", "0.05"))
ENDPOINT_REF_MSE_MAX = float(os.environ.get("V16_ENDPOINT_REF_MSE_MAX", "0.01"))
ACTOR_CQL_COEF = float(os.environ.get("V16_ACTOR_CQL_COEF", "0.1"))
EMPIRICAL_MIN_EPISODES = int(os.environ.get("V16_EMPIRICAL_MIN_EPISODES", "16"))
G_MIN_EMPIRICAL = float(os.environ.get("V16_G_MIN_EMPIRICAL", "0.0"))
ACTOR_BETA = float(os.environ.get("V16_ACTOR_BETA", "2.0"))
TRAIN_REF_DROPOUT = float(os.environ.get("V16_TRAIN_REF_DROPOUT", "0.5"))
CRITIC_REF_DROPOUT = float(os.environ.get("V16_CRITIC_REF_DROPOUT", "0.5"))
# Additive explore off: paper uses π_θ stochasticity after warmup.
EXPLORE_RESIDUAL_STD = float(os.environ.get("V16_EXPLORE_RESIDUAL_STD", "0.0"))

HTTP_PORTS = tuple(range(8710, 8717))
...
REQUIRED_V16_TRAINER_OPTIONS = tuple(
    dict.fromkeys(
        (
...
def _assert_v16_run_dir(run_dir: Path) -> Path:
    ...
    for forbidden in (
        ...
        if forbidden in resolved.parts:
            raise ValueError(f"V16 refuses prior controlled run path: {resolved}")
    return resolved
...
def training_output_dir(run_dir: Path, variant: VariantSpec) -> Path:
    return _assert_v16_run_dir(run_dir) / variant.name
...
def _apply_v16_flags(command: list[str], variant: VariantSpec) -> list[str]:
...
def build_train_command(
    *,
    ...
    run_dir = _assert_v16_run_dir(run_dir)
    ...
    return _apply_v16_flags(command, variant)
...
def validate_trainer_cli(train_script: Path) -> dict[str, Any]:
    ...
    missing = [
        option
        for option in REQUIRED_V16_TRAINER_OPTIONS
        if option not in source
    ]
    return {
        "valid": not missing,
        "train_script": str(train_script),
        "required_options": list(REQUIRED_V16_TRAINER_OPTIONS),
        "missing_options": missing,
    }


def _install_into_v14() -> None:
    """Point v14 CLI helpers at V16 constants/builders."""
    ...
    v14.REQUIRED_V14_TRAINER_OPTIONS = REQUIRED_V16_TRAINER_OPTIONS
    v14._assert_v14_run_dir = _assert_v16_run_dir  # type: ignore[attr-defined]
    ...
    def _pid_belongs_to_run(pid: int, run_dir: Path) -> bool:
        ...
        try:
            ...
            return False
        marker = f"RLT_CF_V16_RUN_DIR={run_dir.resolve()}".encode("utf-8")
