"""V18 harness: success-BC init + longer BC + gate-aware mixture + 2k budget.

Extends V17 with:
- BC 400 + q_ramp 200
- mixture 0.05 pre-gate / 0.25 post-gate (flow pre-gate 0)
- pos_frac 0.5, explore std 0.01
- max 2000 episodes
"""
...
MAX_VALID_EPISODES = int(os.environ.get("V18_MAX_VALID_EPISODES", "2000"))
TARGET_ENV_STEPS = int(os.environ.get("V18_TARGET_ENV_STEPS", "1200000"))
SNAPSHOT_EPISODES = tuple(
    int(token.strip())
    for token in os.environ.get(
        "V18_SNAPSHOT_EPISODES",
        "0,100,200,400,700,1000,1500,2000",
    ).split(",")
    if token.strip()
)
MAX_UPDATE_SEC_PER_EPISODE = float(
    os.environ.get("V18_MAX_UPDATE_SEC_PER_EPISODE", "60")
)
UPDATES_PER_EPISODE = int(os.environ.get("V18_UPDATES_PER_EPISODE", "128"))
BENCHMARK_POSE_CYCLE = TRAIN_EPISODES

# V18 collect / loss defaults (fix V16 always-collect SR collapse).
ACTOR_MIXTURE_PROB = float(os.environ.get("V18_ACTOR_MIXTURE_PROB", "0.05"))
ACTOR_MIXTURE_PROB_PRE_GATE = float(os.environ.get("V18_ACTOR_MIXTURE_PROB_PRE_GATE", str(ACTOR_MIXTURE_PROB)))
ACTOR_MIXTURE_PROB_POST_GATE = float(os.environ.get("V18_ACTOR_MIXTURE_PROB_POST_GATE", "0.25"))
FLOW_MIXTURE_PROB_PRE_GATE = float(os.environ.get("V18_FLOW_MIXTURE_PROB_PRE_GATE", "0.0"))
ACTOR_BC_EPISODES = int(os.environ.get("V18_ACTOR_BC_EPISODES", "400"))
Q_RAMP_EPISODES = int(os.environ.get("V18_Q_RAMP_EPISODES", "200"))
ACTOR_BETA = float(os.environ.get("V18_ACTOR_BETA", "5.0"))
# Delay paper always-collect until empirical gate can fire (effectively off).
ALWAYS_COLLECT_AFTER = int(os.environ.get("V18_ALWAYS_COLLECT_AFTER", "100000"))
ENABLE_ALWAYS_COLLECT = os.environ.get("V18_ALWAYS_COLLECT", "0") == "1"
RESIDUAL_CLIP = float(os.environ.get("V18_RESIDUAL_CLIP", "0.02"))
ADVANTAGE_CLIP = float(os.environ.get("V18_ADVANTAGE_CLIP", "0.05"))
ENDPOINT_REF_MSE_MAX = float(os.environ.get("V18_ENDPOINT_REF_MSE_MAX", "0.01"))
ACTOR_CQL_COEF = float(os.environ.get("V18_ACTOR_CQL_COEF", "0.1"))
EMPIRICAL_MIN_EPISODES = int(os.environ.get("V18_EMPIRICAL_MIN_EPISODES", "16"))
G_MIN_EMPIRICAL = float(os.environ.get("V18_G_MIN_EMPIRICAL", "0.0"))
TRAIN_REF_DROPOUT = float(os.environ.get("V18_TRAIN_REF_DROPOUT", "0.5"))
CRITIC_REF_DROPOUT = float(os.environ.get("V18_CRITIC_REF_DROPOUT", "0.5"))
EXPLORE_RESIDUAL_STD = float(os.environ.get("V18_EXPLORE_RESIDUAL_STD", "0.01"))
POS_FRAC = float(os.environ.get("V18_POS_FRAC", "0.5"))

HTTP_PORTS = tuple(range(8750, 8758))
...
REQUIRED_V18_TRAINER_OPTIONS = tuple(
    dict.fromkeys(
        (
...
def _assert_v18_run_dir(run_dir: Path) -> Path:
    resolved = run_dir.resolve()
    if resolved.name != RUN_NAME:
        raise ValueError(
            f"V18 run directory basename must be {RUN_NAME!r}, got {resolved}"
        )
    for forbidden in (
        ...
        if forbidden in resolved.parts:
            raise ValueError(f"V18 refuses prior run path: {resolved}")
    return resolved
...
def training_output_dir(run_dir: Path, variant: VariantSpec) -> Path:
    return _assert_v18_run_dir(run_dir) / variant.name


def _apply_v18_flags(command: list[str], variant: VariantSpec) -> list[str]:
    ...
    if variant.actor_mode == "rlt":
        ...
        pre_mix = (
            FLOW_MIXTURE_PROB_PRE_GATE
            if variant.cf_mode == "flow"
            else ACTOR_MIXTURE_PROB_PRE_GATE
        )
        _append_option(command, "--actor_mixture_prob_pre_gate", pre_mix)
        _append_option(command, "--actor_mixture_prob_post_gate", ACTOR_MIXTURE_PROB_POST_GATE)
...
def build_train_command(
    *,
    ...
    run_dir = _assert_v18_run_dir(run_dir)
    ...
    return _apply_v18_flags(command, variant)
