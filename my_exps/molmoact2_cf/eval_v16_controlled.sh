#!/usr/bin/env bash
# Read-only evaluation of immutable V16 snapshots into V16 validation outputs.
...
TMP_ROLLOUT_DIR="${TMP_ROLLOUT_DIR:-${B1K_TMP}/molmoact2_rlt_eval_v16_controlled}"
RLT_EGL_LOCK_DIR="${RLT_EGL_LOCK_DIR:-${B1K_TMP}/rlt_egl_locks_v16_controlled}"
V16_EVAL_EPISODES="${V16_EVAL_EPISODES:-}"
V16_EVAL_VARIANTS="${V16_EVAL_VARIANTS:-}"
V16_EVAL_POLICIES="${V16_EVAL_POLICIES:-}"
...
if [[ "$(basename "${RUN_DIR}")" != "rlt_cf_v16_controlled" ]]; then
  echo "[v16-eval] RUN_DIR must be the isolated V16 run: ${RUN_DIR}" >&2
  exit 1
fi
case "${RUN_DIR}" in
  *rlt_cf_v13_controlled*|*rlt_cf_v14_controlled*|*rlt_cf_v15_controlled*)
    echo "[v16-eval] refusing prior controlled run path: ${RUN_DIR}" >&2
    exit 1
    ;;
esac
if [[ ! -x "${PYTHON}" ]]; then
  echo "[v16-eval] Python is not executable: ${PYTHON}" >&2
  exit 1
fi
if [[ ! -f "${RUN_DIR}/MANIFEST.json" ]]; then
  echo "[v16-eval] missing immutable V16 manifest: ${RUN_DIR}/MANIFEST.json" >&2
  exit 1
fi
if [[ ! -f "${VAL_BENCHMARK}/benchmark.json" ]]; then
  echo "[v16-eval] missing held-out validation benchmark: ${VAL_BENCHMARK}" >&2
  exit 1
fi
...
IFS=$'\t' read -r \
  ...
  V16_MAX_UPDATE_SEC_PER_EPISODE \
  < <(
    "${PYTHON}" "${HELPER}" manifest-run-settings \
      --manifest "${RUN_DIR}/MANIFEST.json" \
      --format tsv
  )
V16_SNAPSHOT_EPISODES="${MANIFEST_SNAPSHOT_EPISODES}"
if [[ -z "${V16_EVAL_EPISODES}" ]]; then
  V16_EVAL_EPISODES="${MANIFEST_SNAPSHOT_EPISODES}"
fi
...
IFS=',' read -r -a EVAL_EPISODES <<< "${V16_EVAL_EPISODES}"
if (( ${#EVAL_EPISODES[@]} == 0 )); then
  echo "[v16-eval] V16_EVAL_EPISODES is empty" >&2
  exit 1
fi
for episode in "${EVAL_EPISODES[@]}"; do
  if [[ ! "${episode}" =~ ^[0-9]+$ ]]; then
    echo "[v16-eval] invalid episode filter: ${episode}" >&2
    exit 1
  fi
done

echo "[v16-eval] validating manifest and controlled benchmark"
...
if (( ${#GPU_ARRAY[@]} != 8 )); then
  echo "[v16-eval] manifest does not contain eight GPU assignments" >&2
  exit 1
fi
mapfile -t VARIANT_ROWS < <("${PYTHON}" "${HELPER}" variants --format tsv)
if (( ${#VARIANT_ROWS[@]} != 8 )); then
  echo "[v16-eval] expected eight variants, got ${#VARIANT_ROWS[@]}" >&2
  exit 1
fi
...
validate_variant_filters() {
  [[ -n "${V16_EVAL_VARIANTS}" ]] || return 0
  local -a requested=()
  local requested_variant row found
  IFS=',' read -r -a requested <<< "${V16_EVAL_VARIANTS}"
  for requested_variant in "${requested[@]}"; do
    ...
    if (( found == 0 )); then
      echo "[v16-eval] unknown variant filter: ${requested_variant}" >&2
      return 1
    fi
  done
}
...
validate_policy_filters() {
  [[ -n "${V16_EVAL_POLICIES}" ]] || return 0
  local -a requested=()
  local policy
  IFS=',' read -r -a requested <<< "${V16_EVAL_POLICIES}"
  for policy in "${requested[@]}"; do
    case "${policy}" in
      reference|reference_noise|actor|actor_guide) ;;
      *)
        echo "[v16-eval] unknown policy filter: ${policy}" >&2
        return 1
        ;;
    esac
  done
...
LOCK_FILE="${B1K_TMP}/rlt_cf_v16_controlled_eval.lock"
exec 9> "${LOCK_FILE}"
if ! flock -n 9; then
  echo "[v16-eval] another V16 evaluator holds ${LOCK_FILE}" >&2
  exit 1
fi
...
server_ready() {
  ...
  if ! pid_is_live_v16_owned "${pidfile}"; then
    return 1
  fi
  local response
...
eval_command() {
  ...
  if (( ${#GENERATED_COMMAND[@]} == 0 )); then
    echo "[v16-eval] failed to generate command for ${variant}" >&2
    return 1
  fi
}
...
run_one() {
  ...
  if [[ ! -f "${source_dir}/rlt_cf.pt" || ! -f "${source_dir}/snapshot.json" ]]; then
    echo "[v16-eval] requested immutable snapshot is unavailable: ${source_dir}" >&2
    return 1
  fi
  if [[ "${ae_mode}" == "1" && ! -f "${source_dir}/molmo_ae_lora.pt" ]]; then
    echo "[v16-eval] requested AE adapter snapshot is unavailable: ${source_dir}" >&2
    return 1
  fi
  if "${PYTHON}" "${HELPER}" evaluation-complete \
    --out-dir "${out_dir}" \
    --expected-episodes 12 >/dev/null 2>&1; then
    echo "[v16-eval] skip complete ${variant}/${snapshot_name}/${policy}/seed_${seed}"
    return 0
  fi
  if [[ -e "${out_dir}" ]]; then
    local waited=0
    while (( waited < 7200 )); do
      if "${PYTHON}" "${HELPER}" evaluation-complete \
        --out-dir "${out_dir}" \
        --expected-episodes 12 >/dev/null 2>&1; then
        echo "[v16-eval] adopted in-flight complete ${variant}/${snapshot_name}/${policy}/seed_${seed}"
        return 0
      fi
      if [[ ! -f "${out_dir}/eval.log" ]] || [[ $(( $(date +%s) - $(stat -c %Y "${out_dir}/eval.log") )) -gt 300 ]]; then
        echo "[v16-eval] refusing to overwrite incomplete output: ${out_dir}" >&2
        return 1
      fi
      sleep 15
      waited=$((waited + 15))
    done
    echo "[v16-eval] timed out waiting for in-flight ${out_dir}" >&2
    return 1
  fi
  case "${out_dir}" in
    "${RUN_DIR}/validation/"*) ;;
    *)
      echo "[v16-eval] refusing unsafe validation output path: ${out_dir}" >&2
      return 1
      ;;
  esac
  mkdir -p "${out_dir}"
  ...
  echo "[v16-eval] run ${variant}/${snapshot_name}/${policy}/seed_${seed} gpu=${gpu}"
  env \
  ...
  if ! "${PYTHON}" "${HELPER}" evaluation-complete \
    --out-dir "${out_dir}" \
    --expected-episodes 12 >/dev/null 2>&1; then
    echo "[v16-eval] evaluation did not produce a complete result: ${out_dir}" >&2
    return 1
  fi
}
  ...
  while (( ${#EVAL_PIDS[@]} >= MAX_PARALLEL_EVALS )); do
    ...
    for index in "${!EVAL_PIDS[@]}"; do
      ...
      if ! wait "${EVAL_PIDS[$index]}"; then
        echo "[v16-eval] cell failed: ${EVAL_NAMES[$index]}" >&2
        failed=1
      fi
    done
...
for row in "${VARIANT_ROWS[@]}"; do
  variant="${row%%|*}"
  if ! csv_contains "${V16_EVAL_VARIANTS}" "${variant}"; then
    continue
  fi
  ...
  if [[ -n "${port}" ]] && ! server_ready "${variant}" "${port}"; then
    echo "[v16-eval] V16-owned server is unavailable for ${variant} on ${port}" >&2
    exit 1
  fi
  ...
  for episode in "${EVAL_EPISODES[@]}"; do
    ...
    for policy in "${local_policies[@]}"; do
      if ! csv_contains "${V16_EVAL_POLICIES}" "${policy}"; then
        continue
      fi
...
if (( ${#CELL_QUEUE[@]} == 0 )); then
  echo "[v16-eval] filters selected no evaluation cells" >&2
  exit 1
fi

echo "[v16-eval] scheduling ${#CELL_QUEUE[@]} cells with max_parallel=${MAX_PARALLEL_EVALS} egl_per_gpu=${RLT_EGL_PER_GPU:-2}"
...
echo "[v16-eval] all requested immutable-snapshot evaluations are complete"
