#!/usr/bin/env bash
# V18 residual_rlt_cf + flow_rlt_cf (kettle-matched offline, safe collect).
#   residual_rlt_cf → GPUs 0-3 (4 shards)
#   flow_rlt_cf     → GPUs 4-7 (4 shards)
# Ports 8750–8757. Requires runs/rlt_pretrain_house0_kettle_z256_d512_l4/.
...
RUN_DIR="${RUN_DIR:-${ROOT}/runs/rlt_cf_v18_kettle}"
LOCAL_LOG_DIR="${LOCAL_LOG_DIR:-${B1K_TMP}/rlt_cf_v18_kettle_logs}"
BENCHMARK_ROOT="${BENCHMARK_ROOT:-${ROOT}/runs/benchmarks/house0_kettle_v13}"
PRETRAIN_DIR="${PRETRAIN_DIR:-${ROOT}/runs/rlt_pretrain_house0_kettle_v18_success_bc}"
if [[ ! -d "${PRETRAIN_DIR}" ]]; then
  PRETRAIN_DIR="${ROOT}/runs/rlt_pretrain_house0_kettle_z256_d512_l4"
fi
...
RLT_EGL_LOCK_DIR="${RLT_EGL_LOCK_DIR:-${B1K_TMP}/rlt_egl_locks_v18_kettle}"
TMP_ROLLOUT_DIR="${TMP_ROLLOUT_DIR:-${B1K_TMP}/molmoact2_rlt_rollouts_v18_kettle}"

V18_MODE="${V18_MODE:-long}"
FRESH="${FRESH:-1}"
...
BASE_HTTP_PORT="${BASE_HTTP_PORT:-8750}"
SHARDS_PER_ARM="${SHARDS_PER_ARM:-4}"

case "${V18_MODE}" in
  full)
    V18_MAX_VALID_EPISODES="${V18_MAX_VALID_EPISODES:-400}"
    V18_TARGET_ENV_STEPS="${V18_TARGET_ENV_STEPS:-250000}"
    V18_SNAPSHOT_EPISODES="${V18_SNAPSHOT_EPISODES:-0,100,200,400}"
    ;;
  long)
    V18_MAX_VALID_EPISODES="${V18_MAX_VALID_EPISODES:-2000}"
    V18_TARGET_ENV_STEPS="${V18_TARGET_ENV_STEPS:-1200000}"
    V18_SNAPSHOT_EPISODES="${V18_SNAPSHOT_EPISODES:-0,100,200,400,700,1000,1500,2000}"
    ;;
  smoke)
    V18_MAX_VALID_EPISODES="${V18_MAX_VALID_EPISODES:-2}"
    V18_TARGET_ENV_STEPS="${V18_TARGET_ENV_STEPS:-1000}"
    V18_SNAPSHOT_EPISODES="${V18_SNAPSHOT_EPISODES:-0,${V18_MAX_VALID_EPISODES}}"
    ;;
  *)
    echo "[v18] V18_MODE must be full, long, or smoke" >&2
    exit 1
    ;;
esac

V18_MAX_UPDATE_SEC_PER_EPISODE="${V18_MAX_UPDATE_SEC_PER_EPISODE:-60}"
V18_UPDATES_PER_EPISODE="${V18_UPDATES_PER_EPISODE:-128}"
...
if [[ "$(basename "${RUN_DIR}")" != "rlt_cf_v18_kettle" ]]; then
  echo "[v18] RUN_DIR basename must be rlt_cf_v18_kettle" >&2
  exit 1
fi
if [[ ! -f "${RESIDUAL_CKPT}" || ! -f "${FLOW_CKPT}" ]]; then
  echo "[v18] missing kettle pretrain ckpts under ${PRETRAIN_DIR}" >&2
  echo "  run: bash pretrain_rlt_house0_kettle_d512_l4.sh" >&2
  exit 1
fi
if [[ ! -x "${PYTHON}" || ! -x "${SERVE_PYTHON}" ]]; then
  echo "[v18] python missing" >&2
  exit 1
fi
...
export V18_MODE V18_MAX_VALID_EPISODES V18_TARGET_ENV_STEPS V18_SNAPSHOT_EPISODES
export V18_MAX_UPDATE_SEC_PER_EPISODE V18_UPDATES_PER_EPISODE
export RLT_CF_V18_RUN_DIR="${RUN_DIR}"
...
if (( ${#GPU_ARRAY[@]} != 8 )); then
  echo "[v18] need 8 GPUs, got ${#GPU_ARRAY[@]}" >&2
  exit 1
fi
...
pid_belongs_to_run() {
  ...
  while IFS= read -r entry; do
    [[ "${entry}" == "RLT_CF_V18_RUN_DIR=${RUN_DIR}" ]] && return 0
  done < <(tr '\0' '\n' < "${environ}")
  return 1
}
...
wait_for_server() {
  ...
  echo "[v18] server ${variant} port ${port} not ready" >&2
  return 1
}
...
start_server() {
  ...
  echo "[v18 $(date -Is)] server ${variant} s${shard} gpu=${gpu} port=${port}"
  (
    cd "${MOLMOACT2}"
    exec setsid env \
...
start_trainer() {
  ...
  echo "[v18 $(date -Is)] train ${variant} s${shard} gpu=${gpu} port=${port} fresh=${fresh_start}"
  (
    exec setsid env \
      RLT_CF_V18_RUN_DIR="${RUN_DIR}" \
      CUDA_VISIBLE_DEVICES="${gpu}" \
...
launch_arm() {
  ...
  if (( ${#gpus[@]} != SHARDS_PER_ARM )); then
    echo "[v18] expected ${SHARDS_PER_ARM} GPUs for ${variant}" >&2
    return 1
  fi
  if [[ "${FRESH}" == "1" ]]; then
...
echo "[v18 $(date -Is)] launching residual_rlt_cf on GPUs ${RESIDUAL_GPUS[*]}"
launch_arm residual_rlt_cf "${RESIDUAL_CKPT}" "${RESIDUAL_GPUS[@]}"
echo "[v18 $(date -Is)] launching flow_rlt_cf on GPUs ${FLOW_GPUS[*]}"
launch_arm flow_rlt_cf "${FLOW_CKPT}" "${FLOW_GPUS[@]}"

touch "${RUN_DIR}/.initial_launch_complete"
echo "[v18 $(date -Is)] initial launch complete; polling every ${POLL_SEC}s"
