#!/usr/bin/env bash
# Launch the 24-worker MolmoAct2 RLT-CF v4 comparison in a screen session.
...
RUN_DIR="${RLT_CF_V4_RUN_DIR:-${RUN_DIR:-runs/rlt_cf_v4_screen}}"
LOCAL_LOG_DIR="${LOCAL_LOG_DIR:-/workspace-SR008.nfs2/users/staroverov/B1K/tmp/rlt_cf_v4_logs}"
SCREEN_NAME="${SCREEN_NAME:-rlt_cf_v4}"
NUM_GPUS="${NUM_GPUS:-8}"
...
LOG_EVERY_EPISODES="${LOG_EVERY_EPISODES:-100}"
HORIZON="${HORIZON:-500}"
N_CRITICS="${N_CRITICS:-10}"
SERVER_STAGGER_SEC="${SERVER_STAGGER_SEC:-2}"
TRAINER_STAGGER_SEC="${TRAINER_STAGGER_SEC:-1}"
...
if [[ -z "${UV_BIN}" || ! -x "${UV_BIN}" ]]; then
  echo "[launch] uv executable not found" >&2
  exit 1
fi
...
if [[ "${RLT_V4_IN_SCREEN:-0}" != "1" && "${NO_SCREEN:-0}" != "1" ]]; then
  ...
  screen -dmS "${SCREEN_NAME}" \
    env RLT_V4_IN_SCREEN=1 RLT_CF_V4_RUN_DIR="${RUN_DIR}" \
    bash "${ROOT}/launch_rlt_v4.sh"
...
echo "[launch $(date -Is)] starting RLT-CF v4"
...
start_server() {
  ...
  local -a command=(
    "${UV_BIN}" run python "${ROOT}/serve.py"
    --host 0.0.0.0
    --port "${port}"
    --device cuda:0
    --dtype bfloat16
