#!/usr/bin/env python3
"""Plot DFL-RQL v6 vs RQL baseline on the OGBench 50-task suite."""
...
def report_checkpoints(report_step: int) -> tuple[int, int, int]:
    """Paper protocol: mean success over the last three 100k-spaced eval checkpoints."""
    return (
...
def checkpoint_label(report_step: int) -> str:
...
def load_task_curve(
    ...
    exp_root = exp_root_for_task(save_dir, task, method=method)
...
def collect_multiseed_metrics(
    ...
    checkpoints = report_checkpoints(report_step) if paper_protocol else None
    if paper_protocol:
        indices = {
        ...
        indices = {
            seed: build_result_index(
                save_dir,
                seed,
                prefer_step=report_step if require_exact_step else None,
            )
            for seed in seeds
        }
    methods = ["baseline", "dflrql6"]
    ...
    ckpt_label = checkpoint_label(report_step)
    for task in tasks:
        row: dict = {
            "task_id": task.task_id,
            "domain_id": task.domain_id,
...
def plot_all_tasks_metrics(
    ...
    if common_max_step:
        ...
        ckpt_label = checkpoint_label(report_step)
        protocol_note = (
            f"mean @ {ckpt_label} (paper protocol)"
            if paper_protocol
            else f"@ {report_step // 1000}k steps"
        )
    ...
    for method_label, method, _color in active_methods:
        ...
        lines.append(
            f"{method_label:14s} {mean:.3f} ± {std:.3f}  ({n_seed} seed aggregates)"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=True,
        help="Average success over last three eval checkpoints (800k/900k/1M or 1.8M/1.9M/2M).",
    )
    parser.add_argument(
        "--v7-log-root",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        choices=["baseline", "dflrql6", "dflrql7", "dflrql8", "dflrql9"],
    ...
    if args.common_max_step:
        ...
        proto = checkpoint_label(args.report_step) if args.paper_protocol else str(args.report_step)
