...
class GradientGuidance(nn.Module):
    ...
    W(s, f) is trained to match the unit-normalized action gradient of the
    target critic ensemble mean, so at inference it provides an amortized
    grad_x Q direction without backprop through the deep critic.
    """
...
class DFLRQL4Agent(flax.struct.PyTreeNode):
    """DFL-RQL v4: late-onset unit-direction critic-gradient guidance.

    v1-v3 established that additive guidance drags performance while the
    critic is immature (roughly the first half of training on
    humanoidmaze-large) but produces much steeper late-phase improvement
    slopes than the baseline once the critic is mature. v4 therefore keeps
    v2's verified distillation mechanism (unit direction = implicit trust
    region; cosine to the true critic gradient 0.94-0.98) and moves the
    schedule to a late-onset ramp:

        ramp(step) = clip((step - ramp_start) / (ramp_end - ramp_start), 0, 1)

    Training is bit-identical to plain RQL until `guidance_ramp_start`
    (default 500k), then guidance blends in linearly until
    `guidance_ramp_end` (default 700k) at reduced strength
    (guidance_coef=0.5) for the plateau phase.

    Guided dynamics (used in sampling, reversal, and the q_pe lookahead):

        dx/df = v_theta(x, s, f) + guidance_coef * ramp(step) * f * W(s, f) / ||W(s, f)||
    """
    ...
    def guidance_direction(self, observations, times):
        """Time-gated, warm-up-scaled unit guidance direction."""
    ...
    def total_loss(self, batch, grad_params, rng=None):
        ...
        # Gradient distillation loss: regress W(s, t) onto the unit-normalized
        # action gradient of the target critic ensemble mean at the BC
        # interpolant points. Backprop through the critic happens only here
        # (once per batch), never during flow integration.
        def q_tgt_sum(x):
