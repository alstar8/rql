...
class DFLRQL2Agent(flax.struct.PyTreeNode):
    """DFL-RQL v2: amortized critic-gradient guidance with training warm-up.

    Changes vs v1 (agents/dflrql.py):
    1. The guidance head W(s, f) is trained by distilling the actual action
       gradient of the target critic (unit-normalized), instead of being the
       weight of a global linear fit of Q. This gives a local, state-and-time
       specific steering direction.
    2. Guidance strength ramps in linearly over the first
       `guidance_warmup_steps` training steps, so early training matches the
       plain RQL baseline while W is still being learned.
