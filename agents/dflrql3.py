...
class GradientGuidance(nn.Module):
    """Time-conditioned guidance head: (s, f) -> W in R^d.

    W(s, f) is trained to match the batch-relative action gradient of the
    target critic ensemble mean (raw gradient divided by the batch-mean
    gradient norm), so at inference it provides an amortized grad_x Q with
    relative magnitude, without backprop through the deep critic.
    """
...
class DFLRQL3Agent(flax.struct.PyTreeNode):
    """DFL-RQL v3: magnitude-aware amortized critic-gradient guidance.

    Changes vs v2 (agents/dflrql2.py):
    1. W(s, f) distills the batch-relative raw gradient g / mean_batch(||g||)
       instead of the unit direction g / ||g||. Guidance therefore vanishes
       where the critic is stationary (preserving the BC optimum) and pushes
       hard only where the critic strongly prefers a direction. v1/v2 applied
       a constant-magnitude force regardless of critic preference, which
       dragged actions off the data manifold throughout training.
    2. guidance_coef default halved to 0.5 to further reduce mid-training
       off-manifold drag (v2 lagged baseline by up to 0.31 success at 700k).
    3. guidance_warmup_steps default raised to 300k: the critic only becomes
       informative around 300-400k steps on humanoidmaze-large.

    Guided dynamics (used in sampling, reversal, and the q_pe lookahead):

        dx/df = v_theta(x, s, f) + guidance_coef * ramp(step) * f * W(s, f)

    with ||W|| capped at `guidance_norm_cap` for robustness.
    """
    ...
    def guidance_ramp(self):
        """Linear warm-up of guidance strength over training steps."""
        return jnp.clip(self.network.step / self.config["guidance_warmup_steps"], 0.0, 1.0)

    def guidance_direction(self, observations, times):
        """Time-gated, warm-up-scaled, magnitude-aware guidance velocity."""
        w = self.network.select("guidance")(jnp.concatenate([observations, times], -1))
        w_norm = jnp.linalg.norm(w, axis=-1, keepdims=True)
        cap = self.config["guidance_norm_cap"]
        w = w * jnp.minimum(1.0, cap / (w_norm + 1e-6))
        return self.config["guidance_coef"] * self.guidance_ramp() * times * w
    ...
    def total_loss(self, batch, grad_params, rng=None):
        ...
        for i in range(self.config["flow_steps"]):
            ...
            out = out + self.guidance_direction(batch["observations"][0], f)
        ...
        guided_pred = pred + jax.lax.stop_gradient(
            self.guidance_direction(batch["observations"][0], t)
        )
        ...
        # Gradient distillation loss: regress W(s, t) onto the batch-relative
        # action gradient of the target critic ensemble mean at the BC
        # interpolant points. Dividing by the batch-mean gradient norm keeps
        # the target scale-stable across training while preserving relative
        # magnitude: W ~ 0 where Q is locally flat (critic indifferent).
        ...
        q_grad_rel = q_grad / (q_grad_norm.mean() + 1e-6)

        w = self.network.select("guidance")(
            jnp.concatenate([batch["observations"][0], t], -1), params=grad_params
        )
        distill_loss = (jnp.square(w - q_grad_rel).sum(-1) * valids).mean()
        ...
        return total_loss, {
            ...
            "w_grad_cos": (
                (w * q_grad).sum(-1)
                / (jnp.linalg.norm(w, axis=-1) * q_grad_norm.squeeze(-1) + 1e-6)
            ).mean(),
            "q_grad_rel_norm": jnp.linalg.norm(q_grad_rel, axis=-1).mean(),
    ...
    def compute_flow_actions(
        ...
        for i in range(self.config["flow_steps"]):
            ...
            out = out + self.guidance_direction(observations, t)
    ...
    def create(
        ...
        ex_guidance_in = jnp.concatenate([ex_observations, ex_times], -1)
...
def get_config():
    config = mlc.ConfigDict(
        dict(
            ...
            guidance_coef=0.5,
            distill_coef=1.0,
            guidance_warmup_steps=300000,
            guidance_norm_cap=3.0,
        )
    )
    return config
