...
class LinearGuidance(nn.Module):
    """Time-conditioned linear critic head: (s, f) -> (W in R^d, B in R).

    Q_lin(s, x, f) = W(s, f)^T x + B(s, f). Its action gradient is W(s, f),
    which serves as an analytic guidance direction for the actor flow.
    """
    ...
    def __call__(self, obs_times):
        wb = MLP(
            (*self.hidden_dims, self.action_dim + 1),
...
class DFLRQLAgent(flax.struct.PyTreeNode):
    """RQL with a linear guidance critic (DFL-RQL).

    Identical to RQLAgent except that the actor flow is steered by the
    normalized, time-gated gradient of a linear critic Q_lin = W^T x + B:

        dx/df = v_theta(x, s, f) + guidance_coef * f * W(s, f) / ||W(s, f)||

    W, B are trained by regressing Q_lin at BC interpolant points to the
    target critic ensemble mean (see my_exps/dflrql_method.md).
    """
    ...
    def guidance_direction(self, observations, times):
        """Normalized guidance direction lambda * f * W(s,f)/||W(s,f)||."""
        wb = self.network.select("guidance")(jnp.concatenate([observations, times], -1))
        w = wb[..., :-1]
        w_dir = w / (jnp.linalg.norm(w, axis=-1, keepdims=True) + 1e-6)
        return self.config["guidance_coef"] * times * w_dir
    ...
    def total_loss(self, batch, grad_params, rng=None):
        ...
        # stop-gradded here: the actor loss shapes v_theta only, while W, B
        # are trained by the alignment loss below.
        ...
        # Linear alignment loss: fit Q_lin(s, x_t, t) = W^T x_t + B to the
        # target critic ensemble mean at the same interpolant points, making
        # W an amortized local linear fit (~ grad_x Q) of the value landscape.
        wb = self.network.select("guidance")(
            jnp.concatenate([batch["observations"][0], t], -1), params=grad_params
        )
        w, b = wb[..., :-1], wb[..., -1]
        q_lin = (w * x_t).sum(-1) + b
        q_tgt = self.network.select("target_value")(fm_actor).mean(axis=0)
        align_loss = (jnp.square(q_lin - jax.lax.stop_gradient(q_tgt)) * valids).mean()

        total_loss = (
            actor_loss + bc_loss * self.config["alpha"] + critic_loss
            + align_loss * self.config["align_coef"]
        )
